...
function setupEventListeners() {
  ...
  document.getElementById('update-response-btn')?.addEventListener('click', updateResponse);
  document.getElementById('edit-response-btn')?.addEventListener('click', startEditing);
  document.getElementById('delete-response-btn')?.addEventListener('click', deleteResponse);
...
async function loadSurvey(surveyId) {
  try {
    ...
    const response = await authenticatedFetch(`${API_BASE_URL}/surveys/${surveyId}`);
    ...
    userResponse = currentSurvey.responses?.find(r => r.user._id === userId || r.user === userId);
...
function displaySurvey() {
  ...
  document.getElementById('survey-question').textContent = currentSurvey.guidelines?.question || '';
  ...
  document.getElementById('survey-expiry').textContent = `Expires: ${expiryDate.toLocaleDateString()}`;
  ...
  if (currentSurvey.guidelines?.permittedResponses) {
    document.getElementById('permitted-responses').textContent = currentSurvey.guidelines.permittedResponses;
...
function displayResponseSection(isExpired) {
  ...
  if (isExpired) {
    ...
    if (userResponse) {
      ...
      document.getElementById('response-content').textContent = userResponse.content;
    ...
    if (userResponse) {
      ...
      document.getElementById('response-content').textContent = userResponse.content;
      document.getElementById('edit-response-btn').style.display = 'inline-block';
      document.getElementById('delete-response-btn').style.display = 'inline-block';
      existingResponseDiv.style.display = 'block';
    } else {
      // Show response form for new response
      document.getElementById('submit-response-btn').style.display = 'inline-block';
...
async function submitResponse() {
  ...
  try {
    const response = await authenticatedFetch(`${API_BASE_URL}/surveys/${currentSurvey._id}/responses`, {
      method: 'POST',
...
async function updateResponse() {
  ...
  try {
    const response = await authenticatedFetch(`${API_BASE_URL}/surveys/${currentSurvey._id}/responses/${userResponse._id}`, {
      method: 'PATCH',
      body: JSON.stringify({ content })
    });
...
function createSurveyCard(survey) {
  ...
  const responseCount = survey.responseCount || (survey.responses ? survey.responses.length : 0);
...
function updateDashboardStats(surveys) {
  ...
  if (user) {
    surveys.forEach((survey) => {
      // Check if responses array exists and contains user's response
      if (
        survey.responses &&
  ...
  // Note: With the optimized getSurveys endpoint, user response counting 
  // may not be accurate. For precise counts, individual survey details would be needed.
  ...
  const totalResponses = surveys.reduce(
    (sum, survey) => sum + (survey.responseCount || (survey.responses ? survey.responses.length : 0)),
    0
  );
